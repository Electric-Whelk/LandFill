...
As will be demonstrated in \ref{sec:provscum}, within a number of simulated games that can be run in a user-appropriate timeframe, the chance of two decks scoring comparably when there is only one card different between them, in a way that does not reflect which of the two cards is stronger, is non negligible, essentially introducing a stochastic element on its own. One approach which was explored, however, was a Simulated-Annealing inspired method, using the same search structure, ie, starting off open to some suboptimal decisions and gradually becoming less forgiving to them over time. As trialled in LandFill, this algorithm worked by replacing several lands at once, intially swapping a sizeable proportion of the starting basic manabase with each increment, and accepting fewer and fewer lands each time. 
...
See Fig \ref{fig.progress}. Although initial drafts replaced the "run" button on the Preferences page with the word "Loading", I felt that this was inappropriate for a longer optimization time - I did not wawnt the users to think that the program had simply crashed. Moreover, during mockup testing, several users expressed a desire to see the logic being used by the backend, as this would allow them to trust the results better. Although the Preferences Page contains a contextual explanation of the Hill Climbing algorithm, the use of a Progress page allows for this to be more clearly demonstrated: each step in the Hill Climb increment is printed to screen. At the conclusion of the Hill Climb algorithm, the Output page loads automatically. 
...
\subsubsection{Patterns in Deck Optimization}
\label{sec:patternsindeckoptimization}
