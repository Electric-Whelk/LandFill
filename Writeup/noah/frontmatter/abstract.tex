...
\begin{itemize}
\item I have created a stripped down Magic the Gathering Simulator capable of assigning an approximate performance score to a deck, and from there via Monte Carlo search, a Hill Climbing Optimization routine that uses this data to generate a list of lands for a deck, and a usable interface that allows this to work as a web app. How this system functions is outlined in pages 21-49.
\item In doing so, I have built on existing methods used by professional Magic: The Gathering analysts, notably Frank Karsten, and existing deckbuilding support apps, outlined in pages 8-13.
\item User testing strongly suggests that the resulting product makes it easier to build decks to a higher quality, as outlined in pages 49-54.
