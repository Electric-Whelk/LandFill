...
\item Any spells of \(CMC > L+1\) are ineligible, as they would not be playable in the first place.
\end{itemize}
...
\begin{enumerate}
\item If a Lump is playable on a battlefield containing FilterLands while tapping those FilterLands for C, it is cast as normal.
...
\item Generate a new list, $\underline{L}_1l$, containing all non-Filter Lands, including the subset of Lands capable of paying \(F(1)\)'s cost, \(P_{1}(1) ... P_{1}(m)\). \(P{1}(1)\) is removed from $\underline{L}_1l$, and the SubLands of \(F(1)\) are added in its place. 
...
\item If the Lump is playable with the resulting list of Lands and SubLands, cast it. 
\item If it is not, return the Land \(P_{n}(1)\) that had been removed from $\underline{L}_nl$ and remove \(P_{n}(2)\). Assess the playability of the Lump again, casting it if possible.
\item If no lands \(P_{n}(1)\)-\(P{_n}(m)\) in $\underline{L}_nl$ allow for the lump to be played, return to step 6 for $\underline{L}_{n-1}l$, and so forth for $\underline{L}_{n-2}l$\dots, checking all lists between it and $\underline{L}_nl$ each time.
