...
class DeckBuilder(CardCollection):
    ...
    def run_tests(self):
        ...
        for i in range(0, 1):#SCAFFOLD
            self.set_sample()
            t = MonteCarlo(self.deck, self.cache, close_examine=self._close_examine, timer=self._timer)
            t.run_deck_test()
            self.recall_sample()
    ...
    def anneal(self, temperature, mastertest):
        ...
        while give_up < 100:
            ...
            subtest = MonteCarlo(self.deck, self.cache, close_examine=self._close_examine, timer=self._timer)
            subtest.run_tests()
